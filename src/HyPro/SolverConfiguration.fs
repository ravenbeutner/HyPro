    ...
    | Result.Ok x -> 
        {
            ...
            Ltl2tgbaPath = 
                ...
                |> Option.defaultWith (fun _ -> raise <| HyProException "No field 'ltl2tgba' found")
                |> JSON.tryGetString
                |> Option.defaultWith (fun _ -> raise <| HyProException "Field 'ltl2tgba' must contain a string")
            AutfiltPath = 
                (JSON.tryLookup "autfilt" x)
                |> Option.defaultWith (fun _ -> raise <| HyProException "No field 'autfilt' found")
                |> JSON.tryGetString
...
let getSolverConfiguration() = 
    ...
    if System.IO.FileInfo(solverConfig.Ltl2tgbaPath).Exists |> not then 
        raise <| HyProException "The given path to the spot's ltl2tgba does not exist"

    if System.IO.FileInfo(solverConfig.AutfiltPath).Exists |> not then 
        raise <| HyProException "The given path to the spot's autfilt does not exist"
